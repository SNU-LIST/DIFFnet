...
def main(arg):
    ...
    for dirName, subdirList, fileList in sorted(os.walk(input_location)):
        for filename in fileList:
            if ".mat" in filename.lower():
                ...
                snum_matrix = np.array(io.loadmat(input_location + filename)['snum'], dtype='float')
    ...
    with tf.Session() as sess:
                ...
                for epoch in range(epochs):
                    ...
                    print('loss: %.5f' % (running_loss), end=' ')
                    print("Time: %.3f" %(endTime - startTime), end='  ')
                    print("Lr: %.3e" % (rate * factor))
                    
                    if epoch == 0:
                        save_name = "saving name"
                        temp_d = {'temp_data' : data,'temp_label':label}
                        io.savemat(save_name, temp_d)
